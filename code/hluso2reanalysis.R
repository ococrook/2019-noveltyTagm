...
fData(u2ostagm)$tagm.phenotype <- u2osTagmNoveltyRes$pooledRes$mp$cl[rownames(fData(u2ostagm))]
fData(u2ostagm)$tagm.newcluster.prob <- u2osTagmNoveltyRes$tagm.newcluster.prob[rownames(fData(u2ostagm))]
fData(u2ostagm)$tagm.newcluster.prob[is.na(fData(u2ostagm)$tagm.newcluster.prob)] <- 0
...
addLegend(u2ostagm, fcol = "tagm.mcmc.allocation", ncol = 3, where = "bottomleft", cex = 0.7)
...
# TO DO:
## Number of new allocations plot
## Uncertain between endosome and sectory candidates
## GO enirchment of unclassified proteins
...
gg1 <- plot(u2ostagmNov_conv, tempNames[17]) # Interesting
gg2 <- plot(u2ostagmNov_conv, tempNames[26]) # vescile
gg3 <- plot(u2ostagmNov_conv, tempNames[45]) # known RAB5B
gg4 <- plot(u2ostagmNov_conv, tempNames[62]) # maybe
...
colnames(df) <- c("Markers", "TAGM allocation", "TAGM allocation after reannotation")
...
df_long$Var1[df_long$Var1 == "c"] <- c("Protein annotations after reannotation")
...
gg <- gg + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                 ...
                 text = element_text(size=16)) + scale_fill_manual(values=cbPalette, name = "Legend")
