...
# Set up for heat map
ann_colors = list(
  organelle = c(getStockcol()[1:nlevels(hlmouseTagmNoveltyRes_conv@pooledNoveltyChain@mp@cl)])
)
names(ann_colors$organelle) <- levels(annot$organelle)
...
# Create Heatmap of PSM to big too run subset to 
pheatmap(hlmouseTagmNoveltyRes_conv@pooledNoveltyChain@psm[psmSub,psmSub], useRaster = TRUE, annotation_row = annot, show_rownames = F, show_colnames = F, annotation_colors = ann_colors, treeheight_row = 0, treeheight_col = 0, main = "Heatmap of the posterior similarity matrix for the mouse data", col = myBlues, cellwidth = 2.5)
```
# using pROloc function
```{r,}
...
legend(9, 8, legend = myleg, col = getStockcol(), bty = "n",
       pch = 19, cex = 1, pt.cex = 1)
```
