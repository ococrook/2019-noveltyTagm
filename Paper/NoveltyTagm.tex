...
specialised biological processes to be carried in synchrony. Determining a protein's sub-cellular localisation to one or more compartments is the first step in determining its function. High-throughput and high-accuracy mass spectrometry based sub-cellular proteomic methods can now shed light on the localisation of thousands of proteins at once. Machine learning algorithms are then typically employed to make protein-organelle associations. However, these algorithms make erroneous assignments if data are insufficiently annotated. We develop a semi-supervised Bayesian approach to novelty detection, allowing the discovery of additional sub-cellular niches present within the data. Inference in our model is performed in a fully Bayesian framework and thus we quantify uncertainty in allocation of proteins to new sub-cellular niches, as well as the quantifying the uncertainty in the number of newly discovered compartments. We apply our approach across $9$ mass-spectrometry based spatial proteomic dataset, representing a diverse repertoire of experimental protocols. Application of our approach to two \textit{hyper}LOPIT validates our approach by recovering experimental design. Additional compartmentalisation is uncovered in experiments on the U2OS cell line and this is confirmed by annotations from the human protein atlas. In addition, we uncover a collection of proteins trafficking from the ER to early Golgi in  \textit{Saccharomyces cerevisiae}.
\end{abstract}
